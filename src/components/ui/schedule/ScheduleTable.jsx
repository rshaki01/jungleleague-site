...
export default function ScheduleTable({division, week, teamId, teams}) {
  ...
  return (
    <div className="w-full m-auto mt-5">
        ...
        {games.map((game) => (<div key={game.id}  className="flex flex-col gap-3 p-5 mb-5 bg-gradient-to-br from-gray-800/50 to-gray-900/30 rounded-xl border border-yellow-400/20 overflow-hidden text-xs font-semibold hover:bg-gray-700/30 transition-all duration-200">
            <div className="flex gap-3 border-b border-gray-100/30 mx-auto">
              <div className="text-xl pb-2 text-center">{getTeamById(game.homeTeamId, teams).name}</div>
              <div className="text-xl bg-gradient-to-br from-yellow-400 to-yellow-300 bg-clip-text text-transparent">vs</div>
              <div className="text-xl text-center">{getTeamById(game.awayTeamId, teams).name}</div>
            </div>
